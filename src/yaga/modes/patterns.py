"""Bounded, non-regex matching for committed Git-mode path patterns."""
...
from yaga.modes.models import (
    ...
    _unsafe_revision_character,
    _validate_mode_path,
)

MAX_MODE_PATTERN_BYTES = 512
MAX_MODE_PATTERN_COMPONENTS = MAX_MODE_PATH_COMPONENTS
MAX_MODE_MATCH_WORK = 10_000_000


class ModeMatchWorkLimitError(RuntimeError):
    """Private-to-the-provider signal that deterministic matcher fuel ran out."""


class _MatchWork:
    """One monotonic mode-matcher work budget."""

    __slots__ = ("limit", "remaining")

    def __init__(self, limit: int = MAX_MODE_MATCH_WORK) -> None:
        if isinstance(limit, bool) or not isinstance(limit, int) or limit < 0:
            raise ValueError("mode match-work limit must be a nonnegative integer")
        self.limit = limit
        self.remaining = limit

    def spend(self, units: int) -> None:
        if isinstance(units, bool) or not isinstance(units, int) or units < 0:
            raise AssertionError("mode matcher work cannot be negative")
        if units > self.remaining:
            raise ModeMatchWorkLimitError
        self.remaining -= units


@dataclass(frozen=True, slots=True)
class _CompiledModeComponent:
...
def validate_mode_pattern(pattern: str) -> tuple[str, ...]:
    """Validate one closed mode-policy path pattern."""
    if not isinstance(pattern, str) or not pattern or "\\" in pattern:
        raise ValueError("mode pattern must be a canonical repository-relative POSIX pattern")
    try:
        encoded = pattern.encode("utf-8")
    except UnicodeEncodeError as error:
        raise ValueError("mode pattern must be valid UTF-8") from error

    path = PurePosixPath(pattern)
    components = path.parts
    if (
        len(encoded) > MAX_MODE_PATTERN_BYTES
        or not 1 <= len(components) <= MAX_MODE_PATTERN_COMPONENTS
        or path.is_absolute()
        or bool(PureWindowsPath(pattern).drive)
        or path.as_posix() != pattern
        or any(component in {"", ".", ".."} for component in components)
        or any(_unsafe_revision_character(character) for character in pattern)
    ):
        raise ValueError("mode pattern must be a canonical repository-relative POSIX pattern")
    if any("**" in component and component != "**" for component in components):
        raise ValueError("mode pattern ** wildcard must occupy an entire path component")
    return components


def match_mode_pattern(pattern: str, path: str) -> bool:
    """Return whether one canonical entry path fully matches a validated pattern."""
    pattern_components = validate_mode_pattern(pattern)
    path_components = _validate_mode_path(path)
    return _match_compiled_mode_components(
        _compile_mode_components(pattern_components),
        path_components,
    )


def _compile_mode_components(
    pattern_components: tuple[str, ...],
) -> tuple[_CompiledModeComponent, ...]:
    compiled: list[_CompiledModeComponent] = []
    for component in pattern_components:
        if component == "**":
            compiled.append(_CompiledModeComponent(True, None, (), False, False))
        elif "*" not in component:
            compiled.append(_CompiledModeComponent(False, component, (), False, False))
        else:
            compiled.append(
                _CompiledModeComponent(
                    False,
                    None,
...
def _match_compiled_mode_components(
    pattern_components: tuple[_CompiledModeComponent, ...],
...
def _match_component(
    pattern: _CompiledModeComponent,
    value: str,
    *,
...
def _spend_search_work(spend_work: Callable[[int], None] | None, units: int) -> None:
    if spend_work is not None:
        spend_work(units)
