"""Tests for the installed explicit committed blob-size policy command."""

from __future__ import annotations

import json
import re
import subprocess
from pathlib import Path

import pytest
from typer.testing import CliRunner

from yaga.cli import app
from yaga.commands import size as size_commands
from yaga.errors import GitError
from yaga.sizes.models import (
    SIZE_BLOB_CODE,
    BlobEntry,
    SizeDiagnostic,
    SizePolicy,
    SizeReport,
    SizeSelection,
)
from yaga.sizes.service import CheckedSize

runner = CliRunner()


def _unstyle(value: str) -> str:
    return re.sub(r"\x1b\[[0-?]*[ -/]*[@-~]", "", value)


def _git(repository: Path, *arguments: str) -> str:
    completed = subprocess.run(
        ["git", "-C", str(repository), *arguments],
        check=True,
        capture_output=True,
        text=True,
    )
    return completed.stdout.strip()


def _checked(tmp_path: Path, *, failed: bool) -> CheckedSize:
    size = 20 if failed else 5
    policy = SizePolicy(
        size_policy_version=1,
        default_max_blob_bytes=10,
        max_total_blob_bytes=100,
        path_limits=(),
    )
    blob = BlobEntry(path="README.md", oid="c" * 40, mode="100644", size=size)
    selection = SizeSelection(
        repository=(tmp_path / "repository").resolve(),
        revision="release-candidate",
        commit_sha="a" * 40,
        tree_sha="b" * 40,
        blobs=(blob,),
        gitlinks=(),
    )
    diagnostics = (
        (
            SizeDiagnostic(
                code=SIZE_BLOB_CODE,
                message="committed blob exceeds its configured byte limit",
                size=size,
                limit=10,
                path="README.md",
            ),
        )
        if failed
        else ()
    )
    return CheckedSize(
        report=SizeReport(policy=policy, selection=selection, diagnostics=diagnostics),
        policy_path=(tmp_path / "size-policy.toml").resolve(),
    )


def test_size_help_exposes_only_explicit_policy_revision_and_runtime_options() -> None:
    root = runner.invoke(app, ["--help"])
    group = runner.invoke(app, ["size", "--help"])
    command = runner.invoke(app, ["size", "check", "--help"])

    assert root.exit_code == 0
    assert "size" in root.stdout
    assert group.exit_code == 0
    assert "Check committed blob-size policy" in group.stdout
    assert command.exit_code == 0
    help_text = _unstyle(command.stdout)
    for option in ("--policy", "--revision", "--repo", "--format"):
        assert option in help_text
    for output_format in ("text", "json", "github"):
        assert output_format in help_text
    assert "--quiet" in help_text
    for unsupported in (
        "--current",
        "--event-file",
        "--working-tree",
        "--staged",
        "--content",
        "--token",
    ):
        assert unsupported not in help_text


@pytest.mark.parametrize("missing", ["policy", "revision"])
def test_size_check_requires_both_explicit_inputs(missing: str) -> None:
    arguments = ["size", "check"]
    if missing != "policy":
        arguments.extend(("--policy", "size-policy.toml"))
    if missing != "revision":
        arguments.extend(("--revision", "HEAD"))

    result = runner.invoke(app, arguments)

    assert result.exit_code == 2
    assert f"--{missing}" in _unstyle(result.stderr)


@pytest.mark.parametrize(
    ("failed", "expected_exit", "quiet"),
    [(False, 0, False), (True, 1, False), (False, 0, True), (True, 1, True)],
)
def test_size_check_reports_policy_outcomes_on_stdout(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    failed: bool,
    expected_exit: int,
    quiet: bool,
) -> None:
    checked = _checked(tmp_path, failed=failed)
    captured: dict[str, object] = {}

    def fake_check_size_policy(
        repository: Path,
        *,
        policy_path: Path,
        revision: str,
    ) -> CheckedSize:
        captured.update(repository=repository, policy_path=policy_path, revision=revision)
        return checked

    monkeypatch.setattr(size_commands, "check_size_policy", fake_check_size_policy)
    result = runner.invoke(
        app,
        [
            "size",
            "check",
            "--policy",
            "policy.toml",
            "--revision",
            "release-candidate",
            "--repo",
            str(tmp_path),
            "--format",
            "json",
            *(["--quiet"] if quiet else []),
        ],
    )

    assert result.exit_code == expected_exit, result.stderr
    assert result.stderr == ""
    if quiet:
        assert result.stdout == ""
    else:
        document = json.loads(result.stdout)
        assert document["status"] == ("failed" if failed else "passed")
    assert captured == {
        "repository": tmp_path,
        "policy_path": Path("policy.toml"),
        "revision": "release-candidate",
    }


def test_size_check_defaults_repository_to_current_directory(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    checked = _checked(tmp_path, failed=False)
    captured: dict[str, object] = {}

    def fake_check_size_policy(
        repository: Path,
        *,
        policy_path: Path,
        revision: str,
    ) -> CheckedSize:
        captured.update(repository=repository, policy_path=policy_path, revision=revision)
        return checked

    monkeypatch.setattr(size_commands, "check_size_policy", fake_check_size_policy)
    result = runner.invoke(
        app,
        ["size", "check", "--policy", "policy.toml", "--revision", "HEAD"],
    )

    assert result.exit_code == 0, result.stderr
    assert captured["repository"] == Path(".")


@pytest.mark.parametrize("output_format", ["text", "json", "github"])
def test_size_check_renders_operational_errors_on_stderr(
    monkeypatch: pytest.MonkeyPatch,
    output_format: str,
) -> None:
    monkeypatch.setattr(
        size_commands,
        "check_size_policy",
        lambda *args, **kwargs: (_ for _ in ()).throw(GitError("bad\x1b[31m\nrevision")),
    )

    result = runner.invoke(
        app,
        [
            "size",
            "check",
            "--policy",
            "policy.toml",
            "--revision",
            "missing",
            "--format",
            output_format,
            "--quiet",
        ],
    )

    assert result.exit_code == 2
    assert result.stdout == ""
    assert "\x1b" not in result.stderr
    if output_format == "json":
        document = json.loads(result.stderr)
        assert document["status"] == "error"
        assert document["error"] == {"kind": "git", "message": "bad?[31m?revision"}
    elif output_format == "github":
        assert result.stderr.strip() == (
            "::error title=YAGA size policy::YAGA git error: bad?[31m?revision"
        )
    else:
        assert result.stderr.strip() == "YAGA git error: bad?[31m?revision"


def test_size_check_reads_only_the_exact_committed_blobs(tmp_path: Path) -> None:
    repository = tmp_path / "repository"
    repository.mkdir()
    _git(repository, "init", "--initial-branch=main")
    _git(repository, "config", "user.name", "YAGA Tests")
    _git(repository, "config", "user.email", "yaga@example.invalid")
    (repository / "README.md").write_bytes(b"small\n")
    _git(repository, "add", "--", "README.md")
    _git(repository, "commit", "-m", "chore: establish blob")
    commit_sha = _git(repository, "rev-parse", "HEAD")
    tree_sha = _git(repository, "rev-parse", "HEAD^{tree}")
    (repository / "README.md").write_bytes(b"x" * 4096)
    (repository / "untracked.bin").write_bytes(b"x" * 4096)
    policy = tmp_path / "size-policy.toml"
    policy.write_text(
        "size-policy-version = 1\ndefault-max-blob-bytes = 16\nmax-total-blob-bytes = 16\n",
        encoding="utf-8",
    )

    result = runner.invoke(
        app,
        [
            "size",
            "check",
            "--policy",
            str(policy),
            "--revision",
            "HEAD",
            "--repo",
            str(repository),
            "--format",
            "json",
        ],
    )
    document = json.loads(result.stdout)

    assert result.exit_code == 0, result.stderr
    assert document["identity"] == {
        "policy_path": str(policy.resolve()),
        "repository_path": str(repository.resolve()),
        "revision": "HEAD",
        "commit_sha": commit_sha,
        "tree_sha": tree_sha,
    }
    assert document["counts"] == {"blobs": 1, "gitlinks": 0, "oversized_blobs": 0}
    assert document["total"] == {
        "blob_bytes": 6,
        "max_total_blob_bytes": 16,
        "exceeded": False,
    }


def test_option_like_revision_reaches_bounded_input_error(tmp_path: Path) -> None:
    policy = tmp_path / "size-policy.toml"
    policy.write_text(
        "size-policy-version = 1\ndefault-max-blob-bytes = 16\n",
        encoding="utf-8",
    )

    result = runner.invoke(
        app,
        [
            "size",
            "check",
            "--policy",
            str(policy),
            "--revision=-bad",
            "--repo",
            str(tmp_path),
            "--format",
            "json",
        ],
    )

    assert result.exit_code == 2
    assert result.stdout == ""
    assert json.loads(result.stderr)["error"] == {
        "kind": "input",
        "message": (
            "size revision must be one bounded, non-option commit-ish of at most 512 characters"
        ),
    }
