...
This experimental adapter requires trusted workflows and deployment canaries. For provider-neutral
review plans and receipts, start with the [Agent review guide](agent-review-gate.md).
...
1. [`examples/review-policy.yml`](https://github.com/dariuszpanas/yaga/blob/main/examples/review-policy.yml) is a tiny `pull_request_target`
...
2. Normal unprivileged PR CI runs the repository's tests and ends at a check named
   `CI Prerequisites`.
3. [`examples/agent-review.yml`](https://github.com/dariuszpanas/yaga/blob/main/examples/agent-review.yml) wakes on a completed `CI` or
